...
interface StartupData {
  ...
  funding_amount?: string; // Expected format: "X Lakh INR"
  location?: string;
  tech_stack?: string;
  createdAt?: string;
}
...
const getRecommendedInvestors = async (startupData: StartupData): Promise<Investor[]> => {
  try {
    ...
    const fundingAmountUSD = fundingAmountLakh * 1250; // 1 Lakh INR ≈ $1,250 USD
                  ...
                  {filteredInvestors.length > 0 ? (
                    filteredInvestors.map((investor, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>{`${(investor.checkSize) }0k` }</TableCell>
                        <TableCell>
